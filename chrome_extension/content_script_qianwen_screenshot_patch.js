// 千问截图兜底补丁：
// 千问命中关键词后，强制滚动到目标关键词所在位置，再拉红框并重新截图。
// 解决：回答文本已经命中，但截图页面没有展示目标关键词、红框不在截图内的问题。
(function () {
  ...
  async function forceQianwenKeywordVisible(task, result) {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (typeof waitForScrollStable === "function") {
        await waitForScrollStable(1800);
      } else {
        await sleepLocal(800);
      }
  ...
  function wrapRunner() {
    if (!window.geoAutomationRun || window.__geoQianwenScreenshotPatchWrapped) return;
    const originalRun = window.geoAutomationRun;
    window.geoAutomationRun = async function geoAutomationRunWithQianwenRecapture(task) {
      const result = await originalRun(task);
      try {
        return await recaptureQianwenScreenshot(task, result);
      } catch (error) {
    ...
    window.__geoQianwenScreenshotPatchWrapped = true;
  }
