...
if DESKTOP_SETTINGS.get("output_dir"):
    os.environ["GEO_OUTPUT_DIR"] = str(DESKTOP_SETTINGS["output_dir"])
if DESKTOP_SETTINGS.get("result_excel"):
    os.environ["GEO_RESULT_EXCEL"] = str(DESKTOP_SETTINGS["result_excel"])
...
class DesktopApp:
    def __init__(self, root):
        ...
        self.result_excel = Path(service_config.RESULT_EXCEL)
    ...
    def build_ui(self):
        ...
        output_text = tk.Frame(output_row, bg="white")
        output_text.pack(side="left", fill="x", expand=True, padx=(0, 12))
        ...
        save_desktop_settings(
            {
                ...
                "result_excel": str(self.result_excel),
            }
        )
    ...
    def refresh_output_path(self):
        self.output_path_var.set(
            f"文件夹：{self.output_dir}\n结果：{self.result_excel.name}"
        )

    def service_detail(self):
        return (
            f"输入：{self.input_excel.name}；"
            f"结果：{self.result_excel.name}"
        )
    ...
    def start_service(self):
        ...
        def run():
            try:
                ...
                self.root.after(
                    0,
                    lambda value=message: self.set_status(
                        "error",
                        "服务启动失败",
                        value,
                    ),
                ...
                self.root.after(
                    0,
                    lambda value=str(exc): self.set_status(
                        "error",
                        "服务启动失败",
                        value,
                    ),
    ...
    def validate_input_excel(self, path):
        ...
        if not any(
            name in headers
            for name in service_config.QUESTION_HEADERS
        ):
            supported = "、".join(service_config.QUESTION_HEADERS)
            raise ValueError(
                f"Excel 第一行缺少问题列，支持的列名：{supported}"
            )

    def choose_input_excel(self):
        initial_dir = (
            self.input_excel.parent
            if self.input_excel.parent.exists()
            else Path.home()
        )
        ...
        def apply_selection():
            try:
                ...
                self.result_excel = Path(
                    result.get(
                        "result_excel",
                        service_config.RESULT_EXCEL,
                    )
                )
                ...
                if result.get("resumed_existing_progress"):
                    ...
                    self.detail_var.set(
                        ...
                        f"待继续 {int(stats.get('pending', 0))}"
                    )
                elif result.get("previous_result"):
                    self.detail_var.set(
                        f"已保留旧结果，新的结果将写入：{self.result_excel.name}"
                    )
    ...
    def choose_output_dir(self):
        ...
        def apply_selection():
            try:
                result = service_server.configure_output_dir(selected)

                self.output_dir = Path(result["output_dir"])
                self.result_excel = Path(result["result_excel"])

                # 更换输出目录后，对当前输入表重新初始化任务。
                if self.input_excel.exists():
                    service_server.configure_input_excel(self.input_excel)

                self.refresh_output_path()
                self.save_settings()

                self.detail_var.set(
                    f"新的结果文件：{self.result_excel.name}"
                )

                self.root.after(250, self.start_service)

            except Exception as exc:
                self.set_status("error", "切换输出位置失败", str(exc))
                messagebox.showerror(
                    APP_NAME,
                    f"切换输出位置失败：\n{exc}",
                )
